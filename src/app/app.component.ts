...
export class AppComponent implements OnInit {
  displayedColumns = ['firstName', 'lastName', 'phoneNumber', 'address', 'quantity', 'price', 'actions'];
  ...
  startEdit(i: number, id: number, firstName: string, lastName: string, phoneNumber: string, price: string, address: string, quantity: string) {
    this.id = id;
    // index row is used just for debugging proposes and can be removed
    this.index = i;
    console.log(this.index);
    const dialogRef = this.dialogService.open(EditDialogComponent, {
      data: {id: id, firstName: firstName, lastName: lastName, phoneNumber: phoneNumber, price: price, address: address,quantity:quantity}
    });
  ...
  connect(): Observable<Issue[]> {
    ...
    return merge(...displayDataChanges).pipe(map( () => {
        ...
        this.filteredData = this._exampleDatabase.data.slice().filter((issue: Issue) => {
          const searchStr = (issue.firstName + issue.lastName + issue.phoneNumber ).toLowerCase();
